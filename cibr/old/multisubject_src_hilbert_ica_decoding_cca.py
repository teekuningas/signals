...
def plot_time_series_simple(save_path, data, page, name):
    ...
    for comp_idx in range(data.shape[0]):
        ...
        axes.plot(range(len(series)), series,
                  linewidth=0.5)
...
if __name__ == '__main__':
    ...
    whitened_datasets = []
    whitened_mixings = []
    n_whitening_components = 15
    for subject in subject_data:
        data = subject['data']

        # pca = sklearn.decomposition.PCA(n_components=n_whitening_components, whiten=True)
        # pca.fit(data.T)
        # pca_unmixing = pca.components_
        # whitened_mixings.append(np.linalg.pinv(pca_unmixing))
        # whitened_datasets.append(np.dot(pca_unmixing, data))

        ica_params = {
            'n_components': n_whitening_components,
            'algorithm': 'parallel',
            'whiten': True,
            'max_iter': 10000,
            'tol': 0.000000001
        ...
        whitened_mixings.append(np.linalg.pinv(ica_unmixing))
        whitened_datasets.append(np.dot(ica_unmixing, data))

    blocks = []
    for subject_idx, subject in enumerate(subject_data):
        data = whitened_datasets[subject_idx]
        intervals = subject['intervals']
        name = subject['name']
        ...
        for task in tasks:
            for ival_idx, (key, ivals) in enumerate(intervals.items()):
                if key == task:
                    for ival in [ivals[idx] for idx in 
                                 np.random.permutation(len(ivals))]:
                        x1 = int(ival[0] * sampling_rate_hilbert)
                        x2 = int(ival[1] * sampling_rate_hilbert)
                        import pdb; pdb.set_trace()

                        constructed_set.append(np.mean(data[:, range(x1,x2)], axis=1)[:, np.newaxis])
        blocks.append(np.concatenate(constructed_set, axis=1))

    brain_data = np.vstack(blocks)
    ...
    # brain_pca = sklearn.decomposition.PCA(whiten=True)
    # brain_pca.fit(brain_data.T)
    # brain_pca_unmixing = brain_pca.components_.T
    # brain_pca_mixing = np.linalg.pinv(brain_pca_unmixing)
    # brain_data_wh = np.dot(brain_pca_unmixing, brain_data)
    ...
    # }
    # "sparse"
    sparse_pca = sklearn.decomposition.PCA(n_components=10)
    sparse_pca.fit(brain_data.T)
    sparse_pca_unmixing = sparse_pca.components_
    sparse_pca_mixing = np.linalg.pinv(sparse_pca_unmixing)
    fused_data = np.dot(sparse_pca_unmixing, brain_data)
    ...
    ## brain_ica = FastICA(**ica_params)
    ## brain_ica.fit(brain_data.T)
    ## brain_ica_unmixing = brain_ica.components_

    for subject_idx in range(len(subject_data)):
        subject_mixing = whitened_mixings[subject_idx]
        idxs = range(subject_idx, subject_idx + subject_mixing.shape[1])
        data_to_plot = np.dot(subject_mixing, sparse_pca_mixing[idxs])
        name = 'mixing_' + str(subject_idx+1).zfill(3)

        if src_method == 'vol':
            print("Plotting vol stc brainmaps from ICA mixing matrix..")
            plot_vol_stc_brainmaps(save_path, name, np.abs(data_to_plot.T), vertices,
                                   vol_spacing, subjects_dir)

        else:
            print("Plotting surf stc brainmaps from ICA mixing matrix..")
            plot_stc_brainmaps(save_path, name, np.abs(data_to_plot.T), vertices)

    for component_idx in range(sparse_pca_mixing.shape[1]):
        n_comps = n_whitening_components
        data = [sum(sparse_pca_mixing[:, component_idx][idx*n_comps:(idx+1)*n_comps]**2)
                for idx in range(len(subject_data))]
        data = np.array(data)[np.newaxis, :]

        plot_barcharts(save_path, data, 'subject_weights_' + str(component_idx+1).zfill(2),
                       [str(sub_idx+1).zfill(3) for sub_idx in range(len(subject_data))])

    # plot fused data
    plot_time_series_simple(save_path, fused_data, page, name='fused_data')

    for component_idx in range(sparse_pca_mixing.shape[1]):
        n_comps = n_whitening_components
        comp_dataset = []
        for subject_idx in range(len(subject_data)):
            idxs = range(subject_idx*n_comps, (subject_idx+1)*n_comps)
            subject_pca_unmixing = np.zeros(sparse_pca_unmixing.shape)
            subject_pca_unmixing[:, idxs] = sparse_pca_unmixing[:, idxs]
            subject_series = np.dot(subject_pca_unmixing, brain_data)
            comp_dataset.append(subject_series[component_idx])
        comp_dataset = np.array(comp_dataset)

        name = 'component_' + str(component_idx+1).zfill(2)

        save_folder = None
        if save_path:
            save_folder = os.path.join(save_path, 'cca_series')

        plot_time_series_simple(save_folder, comp_dataset, page, name=name)
    
    import pdb; pdb.set_trace()
    print("miau")
    # exciting
     
    # draw subject specific time series and fused
    # how to test different regulations

    raise Exception('Kissa')

    print("Using iCA")

    brain_icasso = Icasso(FastICA, ica_params=ica_params,
                    iterations=50, bootstrap=False,
                    vary_init=True)
    brain_icasso.fit(brain_data.T, 
                     fit_params={},
                     unmixing_fun=lambda ica: ica.components_)
    brain_ica_unmixing, _ = brain_icasso.get_centrotype_unmixing(distance=0.5)

    fig_ = brain_icasso.plot_dendrogram()
    if save_path:
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        fig_.savefig(os.path.join(save_path, 'dendrogram.png'), dpi=620)

    brain_ica_mixing = np.linalg.pinv(brain_ica_unmixing)
    brain_data_wh = np.dot(brain_ica_unmixing, brain_data)

    if src_method == 'vol':
        print("Plotting vol stc brainmaps from ICA mixing matrix..")
        plot_vol_stc_brainmaps(save_path, 'mixing', np.abs(brain_ica_mixing.T), vertices,
                               vol_spacing, subjects_dir)

    else:
        print("Plotting surf stc brainmaps from ICA mixing matrix..")
        plot_stc_brainmaps(save_path, 'mixing', np.abs(brain_ica_mixing.T), vertices)

    # print "Plotting time series.."
    # plot_time_series(save_path, brain_data_wh, page, sampling_rate_hilbert, 
    #                  subject_data, keys=['mind', 'plan', 'anx'], name='ica_series')

    # EXPERIMENTAL! Zscore individual subjects for ica components
    # brain_data_wh_tmp = []
    # for subject_idx, subject in enumerate(subject_data):
    #     start = int(subject['start'] * sampling_rate_hilbert)
    #     if subject_idx != len(subject_data) - 1:
    #         end = int(subject_data[subject_idx+1]['start'] * sampling_rate_hilbert)
    #     else:
    #         end = int(brain_data_wh.shape[1] - 1)

    #     subj_data = brain_data_wh[:, start:end]
    #     mean_ = np.mean(subj_data, axis=1)[:, np.newaxis]
    #     std_ = np.std(subj_data, axis=1)[:, np.newaxis]
    #     centered = subj_data - mean_
    #     scaled = centered / std_
    #     transformed = scaled + mean_

    #     brain_data_wh_tmp.append(transformed)

    # brain_data_wh = np.concatenate(brain_data_wh_tmp, axis=1)
    # del brain_data_wh_tmp

    print("Plotting time series..")
    plot_time_series(save_path, brain_data_wh, page, sampling_rate_hilbert, 
                     subject_data, keys=['mind', 'plan', 'anx'], name='ica_series')

    def drop_components(dataset, idx_list):
        include = [idx for idx in range(dataset.shape[0]) if idx+1 not in idx_list]
        return dataset[include, :]

    idx_list = []
    import pdb; pdb.set_trace()
    brain_data_wh = drop_components(brain_data_wh, idx_list)
    brain_ica_mixing = drop_components(brain_ica_mixing.T, idx_list).T

    cca_amount = min(len(subject_data[0]['bg_header']), brain_data_wh.shape[0])

    print("Plotting time series..")
    plot_time_series(save_path, brain_data_wh, page, sampling_rate_hilbert, 
                     subject_data, keys=['mind', 'plan', 'anx'], name='ica_series_filtered')

    if src_method == 'vol':
        print("Plotting vol stc brainmaps from ICA mixing matrix..")
        plot_vol_stc_brainmaps(save_path, 'mixing_filtered', np.abs(brain_ica_mixing.T), vertices,
                               vol_spacing, subjects_dir)

    else:
        print("Plotting surf stc brainmaps from ICA mixing matrix..")
        plot_stc_brainmaps(save_path, 'mixing_filtered', np.abs(brain_ica_mixing.T), vertices)

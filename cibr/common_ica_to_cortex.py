...
def visualize_components_in_cortex(inv, maps, raw_info, save_path):
    ...
    while True:
        ...
        fmin = np.percentile(stc.data, 65)
        fmid = np.percentile(stc.data, 93)
        fmax = np.percentile(stc.data, 99)

        brain = stc.plot(hemi='split', views=['med', 'lat'], smoothing_steps=10,
                         surface='white',
...
if __name__ == '__main__':
    ...
    inv = mne.beamformer.make_lcmv(raw.info, fwd, data_cov, reg=0.10, noise_cov=noise_cov)
